...
<nav class="h-menu items-<?= sizeof($links) ?>">
    ...
    <label for="hide-show-menu">
        <img src="./assets/img/hamburger.svg" alt="Hamburger" title="Mostra il menù" class="hamburger" data-inverse-aspect-ratio="1">
        <img src="./assets/img/close.svg" alt="Chiudi" title="Chiudi il menù" class="close" data-inverse-aspect-ratio="1">
    </label>
    <?php foreach ($links as $link) { ?>
        <a href="<?= $link->Url ?>" target="_self" title="Vai a <?= $link->Text ?>">
            <div class="icon">
                <img src="<?= $link->Img ?>" alt="<?= $link->Text ?>" loading="eager" data-inverse-aspect-ratio="1">
            </div>
            <div class="text">
                <span>
                    <?= $link->Text ?>
